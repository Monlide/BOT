...
async def callback_stats(callback: types.CallbackQuery):
    ...
    try:
        await callback.message.edit_media(
            InputMediaPhoto(media=photo, caption=config.get("stats_text", "Статистика временно недоступна")),
            reply_markup=get_back_keyboard()
        )
    except:
        await callback.message.edit_caption(
            caption=config.get("stats_text", "Статистика временно недоступна"),
            reply_markup=get_back_keyboard()
        )
    await callback.answer()
...
async def callback_how(callback: types.CallbackQuery):
    ...
    try:
        await callback.message.edit_media(
            InputMediaPhoto(media=photo, caption=config.get("how_text", "Информация недоступна")),
            reply_markup=get_back_keyboard()
        )
    except:
        await callback.message.edit_caption(
            caption=config.get("how_text", "Информация недоступна"),
            reply_markup=get_back_keyboard()
        )
    await callback.answer()
